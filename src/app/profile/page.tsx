...
const Page = ({ params }: { params: { user: userType } }) => {
  ...
  useEffect(() => {
    if (commande.length === 0) {
      axios
        .get(`/api/commande?userID=${params.user.user_id}`)
        .then((response) => {
              ...
              <div className={styles.general}>
                <div className={styles.profile_picture}>
                  ...
                  {product.map(p => {
                    index++
                    return (
                      <div key={p.name_product}>
                        
                        <img src={p.name_image} alt="image de chaussure" />
                        <h2>{p.name_product}</h2>
                        <p>{p.description_product}</p>
                        <p>Couleur: {p.color_product}</p>
                        <p>Taille: {p.taille_product}</p>
                        <p>Référence: {commande[index].achat_uid}</p>
                        <p>Quantité: {p.quantite_product}</p>
                        <p>Date d'achat: {commande[index].creation_date.toLocaleDateString()}</p>
